function [net, info] = res_lfw224x224(m, varargin)
...
opts.expDir = fullfile('exp', ...
  sprintf('lfw224x224-%s-%d', opts.modelType,m)) ;
else
     opts.expDir = fullfile('exp', ...
  sprintf('lfw224x224-resnet-Pre-%d',m)) ;
end
opts.dataDir = fullfile('/media/wajih/Disk1 500 GB/Onus/RnD/DataSet/face images/','lfw_home') ;
[opts, varargin] = vl_argparse(opts, varargin) ;
...
if opts.preActivation ,
    net = res_lfw224x224_preactivation_init(m) ;
else
    net = res_lfw224x224_init(m, 'networkType', opts.modelType, ...
...
% -------------------------------------------------------------------------
function imdb = getlfw224x224Imdb(opts)
% -------------------------------------------------------------------------
% Preapre the imdb structure, returns image data with mean image subtracted
downloadPath = strcat(opts.dataDir,'/lfw224x244_subset.mat');
...
dataset = dataset.lfw224x244_subset;
fullSetSize = size(dataset,2)*dataset(1).NumberOfImagesPerClass;
traintestRatio = 0.625;
batchSize = 100; 
trainSetSize = ceil(fullSetSize * traintestRatio);
testSetSize = fullSetSize - trainSetSize;
...
labels = cell(1, fullSetSize/batchSize);
sets = cell(1, fullSetSize/batchSize);
...
for i = 1:size(dataset,2)
     for j = 1: trainSetSize/batchSize
     fullTrainingData(:,:,:,i+count1) = double(dataset(i).Images{j})./255.0;
     fullTrainingLabels(i+count1,1) = dataset(i).ClassLabel;
     count1 = count1+1;
     end
     for j= (dataset(1).NumberOfImagesPerClass-testSetSize/batchSize)+1:dataset(1).NumberOfImagesPerClass
     fullTestingData (:,:,:,i+count2) = double(dataset(i).Images{j})./255.0;
     trueLabels(i+count2,1) = dataset(i).ClassLabel;
     count2 =  count2+1;
     end
     count1 = count1-1;
     count2 = count2-1;
...
index = (dataset(1).NumberOfImagesPerClass-testSetSize/batchSize);
