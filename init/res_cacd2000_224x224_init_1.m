function net = res_cacd2000_224x224_init(m, varargin)
switch m,
...
net.meta.trainOpts.learningRate = [0.0001*ones(1,100) 0.0001*ones(1,100) 0.0001*ones(1,100)] ;
net.meta.trainOpts.numEpochs = numel(net.meta.trainOpts.learningRate) ;

% First conv layer
block = dagnn.Conv('size',  [16 16 3 128], 'hasBias', true, ...
                   'stride',3 ,'pad', [1 1 1 1]);
lName = 'conv0';
opts.numEpochs = 40;
net.addLayer(lName, block, 'image', lName, {[lName '_f'], [lName '_b']});

add_layer_bn(net, 128, lName, 'bn0', 0.1);
...
info = add_group(opts.networkType, net, n, info, 3, 128, 4, opts);
info = add_group(opts.networkType, net, n, info, 3, 256, 4, opts);
info = add_group(opts.networkType, net, n, info, 3, 512, 4, opts); 

% Prediction & loss layers

if opts.reLUafterSum 
    block = dagnn.Pooling('poolSize', [2 2], 'method', 'avg', 'pad', 0, 'stride', 1);
